...
const parseTextToQuestions = (text: string, categoryId: string): Question[] => {
    ...
    if (currentQ && currentQ.content) {
      ...
      if (!currentQ.type) {
        if (currentOptions.length > 0) {
          ...
          const match = rawAns.match(/^([A-F\s,]+)/i);
          const cleanAns = match
            ? match[0].replace(/[^A-F]/gi, '').toUpperCase()
            : '';
  ...
  lines.forEach((line) => {
    if (/^(\d+[\.、\)\s\uff0e\u3001]|^\(\d+\))/.test(line)) {
      ...
      const optionRegex = /(^|\s+)([A-F][\.、\)\uff0e\u3001]|\([A-F]\))/g;
      const hasMultipleOptionsInline =
        (line.match(optionRegex) || []).length > 1;

      if (hasMultipleOptionsInline) {
        const splitLine = line
          .replace(optionRegex, '\n$2')
          .split('\n')
          .map((s) => s.trim())
          .filter((s) => s);
        splitLine.forEach((opt) => {
          if (/^([A-F][\.、\)\uff0e\u3001]|\([A-F]\))/.test(opt)) {
            currentOptions.push(opt);
          } else {
            if (currentOptions.length > 0) {
              currentOptions[currentOptions.length - 1] += ' ' + opt;
            }
          }
        });
      } else if (/^([A-F][\.、\)\uff0e\u3001]|\([A-F]\))/.test(line)) {
...
const isJudgmentCorrect = (userAns: string, realAns: string) => {
  ...
  const cleanR = r.replace(/[^A-Z\u4e00-\u9fa5]/g, '').charAt(0);
  if (trueSet.includes(u) && (trueSet.includes(r) || trueSet.includes(cleanR)))
    return true;
  if (
    falseSet.includes(u) &&
    (falseSet.includes(r) || falseSet.includes(cleanR))
  )
    return true;
  return u === r;
};
...
const HighlightIndicator = ({ level }: { level: number }) => {
  ...
  return (
    <div
      className={`w-2.5 h-2.5 rounded-full ${colors[level]} mr-2 inline-block shadow-sm ring-2 ring-white dark:ring-slate-700`}
    ></div>
  );
};
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 relative z-20">
      <div className="flex justify-between items-center px-4 py-3 border-b border-slate-100 dark:border-slate-800">
        <button
          onClick={handleBack}
          className="p-2 -ml-2 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-full transition-colors"
        >
      ...
      {isEditing && (
        <div className="flex items-center gap-3 px-5 py-2 border-y border-slate-50 dark:border-slate-800/50 overflow-x-auto">
          ...
          <button
            ...
            className="w-5 h-5 rounded-full bg-yellow-200 ring-1 ring-black/5"
          ></button>
          <button
            onClick={() => handleHighlight('#bbf7d0')}
            className="w-5 h-5 rounded-full bg-green-200 ring-1 ring-black/5"
          ></button>
          <button
            onClick={() => handleHighlight('#fbcfe8')}
            className="w-5 h-5 rounded-full bg-pink-200 ring-1 ring-black/5"
          ></button>
          <div className="w-px h-4 bg-slate-200 dark:bg-slate-700"></div>
          <button
      ...
      <div
        className={`flex-1 px-5 py-4 overflow-y-auto outline-none text-slate-700 dark:text-slate-300 text-lg leading-relaxed editor-content ${
          !isEditing ? 'read-only' : ''
        }`}
...
const QuestionCard = memo(
  ({
    ...
    const cardBgClass =
      'bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border border-white/20 dark:border-white/10';
    ...
    const renderOption = (opt: string, idx: number) => {
      ...
      let btnClass =
        'border-slate-100 bg-slate-50/50 text-slate-600 hover:bg-slate-100 dark:border-slate-700/50 dark:bg-slate-800/30 dark:text-slate-300 dark:hover:bg-slate-800/50';
      let iconClass =
        'bg-slate-200/80 text-slate-500 dark:bg-slate-700 dark:text-slate-400';

      if (isSelected) {
        btnClass =
          'border-indigo-200 bg-indigo-50 text-indigo-700 shadow-sm ring-1 ring-indigo-200 dark:bg-indigo-900/40 dark:text-indigo-300 dark:border-indigo-700 dark:ring-indigo-800';
      ...
      if ((!data.isExam && data.showAnswer) || data.examSubmitted) {
        ...
        if (isCorrect) {
          btnClass =
            'border-emerald-200 bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-300 dark:border-emerald-700 dark:ring-emerald-800';
          iconClass = 'bg-emerald-500 text-white dark:bg-emerald-600';
        } else if (isSelected) {
          btnClass =
            'border-rose-200 bg-rose-50 text-rose-700 ring-1 ring-rose-200 dark:bg-rose-900/40 dark:text-rose-300 dark:border-rose-700 dark:ring-rose-800';
      ...
      return (
        ...
        >
          <span
            className={`w-6 h-6 flex-shrink-0 flex items-center justify-center rounded-full text-xs font-bold mt-0.5 mr-3 transition-colors ${iconClass}`}
          >
            {optKey}
          </span>
    ...
    return (
      <div className="flex flex-col h-full relative z-10 p-4 pb-24 max-w-2xl mx-auto w-full">
        <div className="flex justify-between items-center mb-4">
          <button
            onClick={onExit}
            className="text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white flex items-center bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-2.5 pr-4 rounded-full shadow-sm transition-all active:scale-95"
          >
            <ChevronLeft size={20} />{' '}
            <span className="text-sm font-bold ml-1">退出</span>
          </button>
          ...
          <div className="flex-1 mx-4 h-1.5 bg-slate-200/50 dark:bg-slate-700/50 rounded-full overflow-hidden backdrop-blur-sm">
            <div
              ...
              style={{
                width: `${((data.index + 1) / data.list.length) * 100}%`,
              }}
            ></div>
          </div>

          <div
            className={`text-slate-500 dark:text-slate-400 font-mono font-bold text-xs px-2`}
          >
        ...
        <div
          className={`flex-1 overflow-y-auto p-6 md:p-8 rounded-[2rem] shadow-xl dark:shadow-black/20 transition-all duration-300 ${cardBgClass}`}
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              <span
                className={`text-[10px] px-2.5 py-1 rounded-lg font-bold tracking-wider uppercase shadow-sm ${getQuestionTypeStyle(
                  q.type
                )}`}
              >
            ...
            <div className="flex items-center gap-2">
              {isWrong && (
                <span className="text-[10px] font-bold text-rose-500 bg-rose-50 dark:bg-rose-900/30 px-2 py-1 rounded-lg flex items-center">
                  <AlertTriangle size={10} className="mr-1" /> 错 {wrongCount}{' '}
                  次
                </span>
              )}
              <div className="flex bg-slate-100/50 dark:bg-slate-800/50 rounded-full p-1">
                <button
                  ...
                  className={`p-1.5 rounded-full transition-all ${
                    highlightLevel > 0
                      ...
                      : 'hover:bg-white/50 dark:hover:bg-slate-700/50 text-slate-400'
                  }`}
                >
                  <div
                ...
                <button
                  ...
                  className={`p-1.5 rounded-full transition-all ${
                    isBookmarked
                      ? 'bg-white dark:bg-slate-700 shadow-sm text-amber-400'
                      : 'hover:bg-white/50 dark:hover:bg-slate-700/50 text-slate-300 dark:text-slate-600'
                  }`}
                >
                  <Star
                    size={16}
                    className={isBookmarked ? 'fill-amber-400' : 'currentColor'}
                  />
                </button>
              </div>
            </div>
          ...
          <div
            ...
            style={{ textShadow: '0 1px 1px rgba(0,0,0,0.02)' }}
          >
          ...
          {q.type === 'judgment' && (
            <div className="flex gap-4 h-24 md:h-32">
              {['√', '×'].map((opt, idx) => {
                ...
                let btnClass =
                  'bg-slate-50 border-slate-200 text-slate-400 hover:bg-slate-100 dark:bg-slate-800/50 dark:border-slate-700 dark:text-slate-500 dark:hover:bg-slate-800';

                if (isSelected) {
                  btnClass =
                    opt === '√'
                      ? 'bg-emerald-50 border-emerald-200 text-emerald-600 dark:bg-emerald-900/40 dark:border-emerald-800 dark:text-emerald-400 shadow-sm'
                      : 'bg-rose-50 border-rose-200 text-rose-600 dark:bg-rose-900/40 dark:border-rose-800 dark:text-rose-400 shadow-sm';
                }
          ...
          {(data.showAnswer || data.examSubmitted) && (
            <div className="mt-8 p-6 bg-emerald-50/50 dark:bg-emerald-900/20 rounded-2xl border border-emerald-100 dark:border-emerald-800/50 animate-in fade-in slide-in-from-bottom-4">
              ...
              <div
                className={`text-slate-800 dark:text-slate-200 font-bold whitespace-pre-wrap ${optionSize}`}
              >
                {q.answer}
              </div>
        ...
        <div className="fixed bottom-6 left-6 right-6 z-30 safe-area-bottom">
          <div
            className={`flex justify-between items-center p-2 rounded-full shadow-xl border border-white/20 dark:border-white/10 ${cardBgClass}`}
          >
            <button
            ...
            <div className="flex-1 flex justify-center px-4">
              ...
              {data.isExam && !data.examSubmitted ? (
                isLastQuestion ? (
                  ...
                  <div className="text-xs text-slate-400 dark:text-slate-500 font-medium flex flex-col items-center">
                    <span className="mb-1">考试进行中</span>
                    <span className="w-12 h-1 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-indigo-500"
                        style={{
                          width: `${
                            ((data.index + 1) / data.list.length) * 100
                          }%`,
                        }}
                      ></div>
                    </span>
